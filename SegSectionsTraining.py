...
def labelData(imgDirs, classes = ["m", "d", "v"]):
    ...
    for n in featIDOrder:
        ...
        imgComb = np.hstack([refImg, sectsStackR])
...
def getLabels(imgDir, infoDir):
    ...
    vals = txtToDict(imgDir + "labels.txt", typeID = int, typeV = str)[0]
...
def VGG16Maker(IMAGE_SIZE, noClasses, Trainable = False, Weights = 'imagenet', Top = False):
    ...
    # NOTE make the last couple of convolutional layers trainable as well
...
if __name__ == "__main__":
    ...
    imgDirs = '/Volumes/USB/H653A_11.3/3/FeatureSections/linearSect_png_False/'
    ...
    # train the model
    modelTrainer(destDir, "VGG16", epochs=5)
